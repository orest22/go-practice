...
func TestClock(t *testing.T) {
	t.Run("test normal behavior", func(t *testing.T) {
		...
		monkey.Patch(time.Now, func() time.Time {
			nowTime, err := time.Parse(layout, "16 Jul 2020 10:03:00")
			if err != nil {
				t.Fatal(err)
			}
			return nowTime
		...
		monkey.Patch(ntp.Time, func(_ string) (time.Time, error) {
			ntpTime, err := time.Parse(layout, "16 Jul 2020 10:03:02")
			if err != nil {
				t.Fatal(err)
			}
			return ntpTime, nil
		...
		expected := `current time: 2020-07-16 10:03:00 +0000 UTC
exact time: 2020-07-16 10:03:02 +0000 UTC
`
